...
(function() {
    ...
    const targets = document.querySelectorAll('div.canvas');
    for (const canvas of targets) {
        const fn = canvas.dataset.fn;
        const args = canvas.dataset.args;
        const options = canvas.dataset.options;

        const func = eval(fn);
        createDynamicGraph(
            canvas,
            func,
            JSON.parse(decodeURIComponent(args)),
